...
import { mtops as mtopsCol } from "@/data";
import { spToLegacyMtop } from "@/lib/prangkisaAdapt";

/**
 * Resolve a credential from the DB-hydrated `mtops` list, falling back to a
 * just-issued application still in the local store (covers the fire-and-forget
 * write race right after issuance, before the row is re-fetched).
 */
function resolveMtop(
  no: string | undefined,
  mtops: Mtop[],
  store: Store,
): Mtop | null {
...
export function MtopView({
  ...
  const copy = useCopy();
  const m = resolveMtop(params.no, mtops, store);
  if (!m) {
    return (
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <CredentialCard
        kicker={`Republic of the Philippines · ${cfg.municipality.fullName}`}
        ...
        footer={
          <>
            ...
            <span>
              Issued via the LGU {cfg.municipality.shortName} Sanggunian App · revocable privilege under {copy("revenueOrdinance", cfg.defaultLang)}, not a right.
            </span>
          </>
        }
      />
...
/* ---------- PUBLIC VERIFY ---------- */
/** Roadside check against the DB-backed verify endpoint (public, no auth). */
async function fetchMtop(no: string): Promise<Mtop | null> {
  // Offline-capable: resolve against the locally-synced franchise collection.
  const m = mtopsCol.all().find((x) => x.ref.toLowerCase() === no.trim().toLowerCase());
  return m ? spToLegacyMtop(m) : null;
}

export function Verify({ go, params }: { go: Go; params: RouteParams }) {
  const cfg = useSpConfig();
  const [q, setQ] = useState(params.no || "");
  // undefined = not searched yet; null = searched, no match; Mtop = found.
  const [result, setResult] = useState<Mtop | null | undefined>(undefined);

  const run = async (val?: string) => {
    const no = (val ?? q).trim();
    if (no) setResult(await fetchMtop(no));
  };

  // Auto-run when arriving with a scanned franchise number (?no=...).
  useEffect(() => {
    if (!params.no) return;
    let active = true;
    void fetchMtop(params.no).then((m) => {
      if (active) setResult(m);
    });
    return () => {
      active = false;
    };
    ...
    <div className="mx-auto max-w-xl space-y-6">
      ...
      <div className="rounded-xl border bg-card p-5 shadow-sm">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {["STC-Z6-0052", "STC-Z2-0418", "STC-Z9-9999"].map((s) => (
            <Button
              key={s}
              variant="outline"
              size="sm"
      ...
      {result === null && (
        <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-6 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            &ldquo;{q}&rdquo; is not an active franchise on record in {cfg.municipality.shortName}. Flag for inspection.
          </p>
        </div>
      )}
          ...
          <div className="flex items-center gap-3 border-b border-primary/20 bg-primary/5 px-5 py-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-muted-foreground">
                Verified against current LGU {cfg.municipality.shortName} records
              </div>
            </div>
