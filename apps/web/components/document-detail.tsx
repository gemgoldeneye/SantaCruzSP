...
export function DocumentDetail({ doc }: { doc: LegislativeDocument | null }) {
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <pre className="mt-1 max-h-96 overflow-auto rounded-lg border bg-muted/40 p-3 text-xs whitespace-pre-wrap">
              {documentBody(doc, {
                country: cfg.municipality.country,
                province: cfg.municipality.province,
                municipality: cfg.municipality.name,
                council: copy("councilName", cfg.defaultLang, cfg.municipality.fullName),
                locale: `${cfg.municipality.shortName}, ${cfg.municipality.province}`,
              })}
            </pre>
          </div>
        </CardContent>
