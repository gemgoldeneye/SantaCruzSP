...
// audit_events carries tenant RLS — every audit write needs the tenant set.
const auditInTenant = (input: AuditInput) => withTenant(input.tenantId, (tx) => audit(input, tx));
...
export async function authRoutes(app: FastifyInstance): Promise<void> {
  app.post('/auth/staff/login', async (req, reply) => {
    ...
    const tenantId = body.data.tenantId ?? env.defaultTenant;

    const row = await db.query.users.findFirst({
      where: and(eq(users.tenantId, tenantId), eq(users.username, body.data.username)),
    });
    ...
    if (!row || !ok || row.status !== 'active') {
      await auditInTenant({ tenantId, actorName: body.data.username, action: 'auth.fail' });
    ...
    const sessionId = await createSession({ userId: row.id, tenantId, user });

    await auditInTenant({ tenantId, actorId: row.id, actorName: row.name, actorRole: row.role, action: 'auth.login' });
    return reply
