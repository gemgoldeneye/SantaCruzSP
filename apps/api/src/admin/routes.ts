...
export async function adminRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post('/api/admin/accounts', async (req, reply) => {
    ...
    const role = await db.query.roles.findFirst({ where: eq(roles.id, body.data.roleId) });
    if (!role) return reply.code(400).send({ error: 'Unknown role.' });

    const existing = await db.query.users.findFirst({ where: eq(users.username, body.data.email) });
    if (existing) return reply.code(409).send({ error: 'An account with that email already exists.' });

    const id = uuidv7();
    await db.insert(users).values({
      id, username: body.data.email, name: body.data.name, role: role.roleKey,
    ...
    await withTenant((tx) => audit({ actorId: req.user!.id, actorName: req.user!.name, actorRole: req.user!.role, action: 'admin.account.create', detail: { email: body.data.email } }, tx));
  ...
  app.patch('/api/admin/accounts/:id', async (req, reply) => {
    ...
    await applyRoleToUser(id, role);
    await withTenant((tx) => audit({ actorId: req.user!.id, actorName: req.user!.name, actorRole: req.user!.role, action: 'admin.account.update', docId: id }, tx));
    return { ok: true };
  });
  ...
  app.delete('/api/admin/accounts/:id', async (req, reply) => {
    ...
    await db.delete(users).where(eq(users.id, id));   // memberships cascade
    await withTenant((tx) => audit({ actorId: req.user!.id, actorName: req.user!.name, actorRole: req.user!.role, action: 'admin.account.delete', docId: id }, tx));
    return { ok: true };
  });
  ...
  app.post('/api/admin/roles', async (req, reply) => {
    ...
    const dup = await db.query.roles.findFirst({ where: eq(roles.key, key) });
    ...
    const inserted = await db.insert(roles).values({
      key, name: body.data.name, description: body.data.description, isStaff: body.data.isStaff, isSystem: false,
      permissions: body.data.permissions, roleKey: rbac.roleKey, offices: rbac.offices, memberships: rbac.memberships,
    }).returning();
    await withTenant((tx) => audit({ actorId: req.user!.id, actorName: req.user!.name, actorRole: req.user!.role, action: 'admin.role.create', detail: { key } }, tx));
  ...
  app.patch('/api/admin/roles/:id', async (req, reply) => {
    ...
    await withTenant((tx) => audit({ actorId: req.user!.id, actorName: req.user!.name, actorRole: req.user!.role, action: 'admin.role.update', docId: id }, tx));
    return { ok: true };
  });
  ...
  app.delete('/api/admin/roles/:id', async (req, reply) => {
    ...
    await withTenant((tx) => audit({ actorId: req.user!.id, actorName: req.user!.name, actorRole: req.user!.role, action: 'admin.role.delete', docId: id }, tx));
    return { ok: true };
  });

  // ── Activity log ────────────────────────────────────────────────────────────
  // The append-only, hash-chained audit trail (platform.audit_events): every staff
  // mutation, login, and admin action. Newest first.
  app.get('/api/admin/audit', async (req) => {
    ...
    return withTenant(async (tx) => {
      const rows = await tx.execute(rawSql`
        ...
        WHERE (${actionFilter} = '' OR action ILIKE ${'%' + actionFilter + '%'})
