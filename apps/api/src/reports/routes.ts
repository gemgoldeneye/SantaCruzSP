...
export async function reportRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/reports/analytics', async (req) => {
    ...
    return withTenant(tenantId, async (tx) => {
      ...
      const docsByStage = rows<{ stage: string; count: number }>(await tx.execute(rawSql`
        ...
        FROM data.documents
        WHERE tenant_id = ${tenantId} AND collection = 'sp.sanggunian.documents' AND deleted_at IS NULL
      ...
      const mtopByStatus = rows<{ status: string; count: number }>(await tx.execute(rawSql`
        ...
        FROM data.documents
        WHERE tenant_id = ${tenantId} AND collection = 'sp.mtop.mtops' AND deleted_at IS NULL
        GROUP BY 1 ORDER BY count DESC
      `));
      ...
      const appsByStage = rows<{ stage: string; count: number }>(await tx.execute(rawSql`
        ...
        FROM data.documents
        WHERE tenant_id = ${tenantId} AND collection = 'sp.mtop.applications' AND deleted_at IS NULL
      ...
      const revenue = rows<{ total: number }>(await tx.execute(rawSql`
        ...
        FROM data.payment_orders WHERE tenant_id = ${tenantId}
      `))[0] ?? { total: 0 };
      ...
      const totals = rows<{ documents: number; sessions: number; feedback: number }>(await tx.execute(rawSql`
        ...
        FROM data.documents WHERE tenant_id = ${tenantId} AND deleted_at IS NULL
