  ...
  return (
    <div className="space-y-3 lg:sticky lg:top-4">
      ...
      <p className="text-center text-xs text-muted-foreground">
        Issued only after SB session approval. Fees are illustrative — confirm vs. the Tricycle Franchising Ordinance.
      </p>
    </div>
  );
}
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Santa Cruz&apos;s charter lists four front-line franchising services. Pick one.
        </p>
      </div>
...
function StepDetails({ data, set }: { data: FormData; set: (p: Partial<FormData>) => void }) {
    ...
    <div className="space-y-5">
      {linked && (
        <div className="flex items-start gap-3 rounded-lg border border-primary/30 bg-primary/5 p-3 text-sm">
          ...
          <p>
            Linked to franchise <span className="font-semibold">{data.franchise || "STC-PS-0418"}</span>.
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <LabelledField label="Address" req>
              <Input value={data.address} onChange={(e) => set({ address: e.target.value })} placeholder="Barangay, Santa Cruz, Zambales" />
            </LabelledField>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            the Tricycle Franchising Ordinance.
          </p>
        </div>
      ...
      {missing.length === 0 ? (
        ...
        <div className="flex items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/5 p-3 text-sm">
          ...
          <p>
            ...
            Under RA 11032, Santa Cruz validates complete-or-return-once — upload everything before submitting.
          </p>
        </div>
      )}
        ...
        {fees.map((f) => (
          <div key={f.k} className="flex items-center justify-between gap-3 py-1.5 text-sm">
            <span className="flex items-center gap-2">
              {f.k}
              {f.confirm && <Badge variant="outline">confirm</Badge>}
            </span>
...
export function ApplyWizard({
  ...
  const prefilled = useMemo<FormData>(() => {
    const seed: FormData = {
      ...
      plate: "",
      chassis: "",
      franchise: params.franchise || "STC-PS-0418",
      uploaded: {},
    };
