...
export function DocumentScanner() {
  ...
  const runOcr = async () => {
    ...
    try {
      ...
      if (match) {
        ...
        setReferenceNo(ref);
        const existing = findByReference(ref);
  ...
  const save = () => {
    ...
    // Dedupe: reference number first, then title.
    const existing =
      (referenceNo.trim() && findByReference(referenceNo)) ||
      findByTitle(cleanTitle);
    if (existing) {
      window.location.href = `/documents/${existing.id}`;
      return;
    }

    const id = `scan-${Date.now()}`;
    const today = new Date().toISOString().slice(0, 10);
    const yearMatch = referenceNo.match(/(19|20)\d{2}/);
    const doc: LegislativeDocument = {
      id,
      referenceNo:
        referenceNo.trim() || `SCAN-${today}-${String(Date.now()).slice(-4)}`,
      title: cleanTitle,
      type,
      category,
      authorIds: [],
      ...
      summary:
        ...
        "Scanned document (no preview text).",
      stage: "filed",
          ...
          {phase === "done" || text ? (
            <>
              ...
              <Button onClick={save} disabled={!title.trim()}>
                <CheckCircle2 className="size-4" /> Save digital copy
              </Button>
            </>
          ) : (
