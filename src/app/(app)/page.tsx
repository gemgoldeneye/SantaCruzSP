...
import { DOCUMENTS, SESSIONS, authorNames } from "@/lib/mock-data";
...
export default function DashboardPage() {
  ...
  const searchablePct = Math.round((searchable.length / totalDocs) * 100);

  const nextSession = SESSIONS.filter((s) => s.status === "scheduled").sort(
    (a, b) => a.date.localeCompare(b.date),
  )[0];

  const pipeline = ACTIVE_STAGES.map((stage) => ({
    stage,
    count: DOCUMENTS.filter((d) => d.stage === stage).length,
  }));
  ...
  const stats = [
    {
      label: "Total records",
      value: totalDocs,
      hint: `${scanned.length} digitized from scans`,
      icon: FileText,
    ...
    },
    {
      ...
      value: activeMeasures.length,
      hint: "in the legislative pipeline",
      icon: GitBranch,
      spark: [2, 3, 3, 5, 4, 6, 7],
      featured: true,
    },
    {
      label: "OCR backlog",
      value: ocrBacklog.length,
      hint: "scanned docs awaiting / failed OCR",
      icon: ScanLine,
      spark: [5, 5, 4, 4, 3, 3, 2],
      featured: false,
    },
    {
      label: "Full-text searchable",
      value: `${searchablePct}%`,
      hint: `${searchable.length} of ${totalDocs} records`,
      icon: SearchCheck,
      spark: [58, 63, 68, 72, 76, 80, 85],
      featured: false,
    },
  ];
  ...
  return (
    <>
      <PageHeader
        ...
        description="Real-time overview of Santa Cruz's legislative records, pipeline, and sessions."
      />
        ...
        {stats.map((s) => (
          <Card
            ...
            className={cn(
              "gap-2",
              s.featured &&
                "shadow-xl shadow-black/30 ring-1 ring-[#caa14a]/40 lg:-translate-y-2",
            )}
          >
            ...
            <CardContent>
              <div
                className={cn(
                  ...
                  s.featured ? "text-[#e3c069]" : "text-foreground",
                )}
              >
                {s.value}
              </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <CalendarClock className="size-4" /> Upcoming session
            </CardTitle>
            {nextSession ? (
              <CardDescription className="capitalize">
                {formatDate(nextSession.date)} · {nextSession.mode}
              </CardDescription>
            ) : null}
          </CardHeader>
          <CardContent>
