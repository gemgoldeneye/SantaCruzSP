#!/usr/bin/env bash
# Runs ON THE PRODUCTION VPS. Builds the 2-app SantaCruzSP stack (Fastify `api` +
# Next.js `web`), migrates + seeds the managed DB once, and rolls the
# stcz-sp Swarm stack. Jenkins passes every value as an ENV VAR (secrets from
# credentials, the rest from params); this script WRITES deploy/prod/.env
